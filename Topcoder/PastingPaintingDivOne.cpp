...
public:
	vector<long long> countColors(vector <string> clipboard, int T) {
	  vector <LL> v;
    v.clear();
    LL r=0,g=0,b=0,R=0,B=0,G=0;
    LL n,m;
    n = clipboard.size();
    m = clipboard[0].length();
    for(LL i=0;i<n;i++){
      for(LL j=0;j<m;j++){
        if(clipboard[i][j]=='R')r++;
        if(clipboard[i][j]=='G')g++;
        if(clipboard[i][j]=='B')b++;
        if(clipboard[i][j]=='.' && i<n-1 && j<m-1){
        	if(clipboard[i+1][j+1]=='R')r++;
        	if(clipboard[i+1][j+1]=='G')g++;
        	if(clipboard[i+1][j+1]=='B')b++;
        }
      }
    }
    cout << r << " "  << g  << " " << b << endl;
    for(LL i=0;i<n;i++){
        if(clipboard[i][0]=='R')R++;
        if(clipboard[i][0]=='G')G++;
        if(clipboard[i][0]=='B')B++;
    }
    for(LL i=1;i<m;i++){
        if(clipboard[0][i]=='R')R++;
        if(clipboard[0][i]=='G')G++;
        if(clipboard[0][i]=='B')B++;
    }
    cout << R << " "  << G  << " " << B << endl;
    r += (T-1)*R;
    g += (T-1)*G;
