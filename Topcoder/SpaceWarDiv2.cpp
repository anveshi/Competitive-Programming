...
class SpaceWarDiv2 {
public:
	bool fun(vector <int> &magicalGirlStrength, vector <pair <int, int> > &v, int t){
		int n = magicalGirlStrength.size();
		int m = v.size();
		vector <int> Res(n, 0);
		for(int i=0;i<m;i++){
			int power = v[i].first, nos = v[i].second; 
			for(int j=0;j<n;j++){
				if(magicalGirlStrength[j] >= power){
					if(Res[j] < t){
						int maxAdd = min(nos, t - Res[j]);
						if(maxAdd >= nos){
							nos = 0;
							Res[j] += maxAdd;
						}
	...
	int minimalFatigue(vector <int> magicalGirlStrength, vector <int> enemyStrength, vector <int> enemyCount) {
		...
		for(int i=0;i<enemyStrength.size();i++){
			v.push_back({enemyStrength[i], enemyCount[i]});
		}
		sort(v.begin(), v.end());
		if(*max_element(magicalGirlStrength.begin(), magicalGirlStrength.end()) < 
			*max_element(enemyStrength.begin(), enemyStrength.end()))
				return -1;
		int st = 0, end = 10000000;
		while(end - st > 1){
			//cout << st << " " << end << endl;
			int mid = (st + end) >> 1;
			if(fun(magicalGirlStrength, v, mid))
				end = mid;
			else
				st = mid ;
		}
		if(fun(magicalGirlStrength, v, st))
			return st;
		return end;
	}
};
